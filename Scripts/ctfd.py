...
import os, sys, shutil, requests, hashlib
...
try:
	...
	for cid in sorted(chall["id"] for chall in challs):
		...
		desc, path = description.strip().replace("\r", ""), f"{save}/{category.strip().replace('/', '-')}/{name.strip().replace('/', '-')}"
	...
	print("Usage: ctfd <host> <session> [--save=<directory>] [--debug]")
	exit("Example: ctfd https://fbctf.com 1234abcd-5e6f-a7b8-9c0d-ef1234567890")
except KeyError:
